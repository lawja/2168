...
{       
      ...
      int srow, int scol, int erow, int ecol, LinkedList<Point> L)
    {
        ...
        LinkedList<Point> Q=new LinkedList<Point>(); Q.addFirst(u);         
        while ((Q.size()!=0)&&(!done))
        {             
           ...
           {  
              if ((r>1)&&(V[r-1][c]!=1)&&(maze.can_go(r, c,'U'))) 
                 {V[r-1][c]=1; P[(r-2)*C+c]=u; Q.addFirst(new Point(r-1, c));}

              if ((c<C)&&(V[r][c+1]!=1)&&(maze.can_go(r, c,'R')))           
                 {V[r][c+1]=1; P[(r-1)*C+c+1]=u; Q.addFirst(new Point(r, c+1));}

              if ((r<R)&&(V[r+1][c]!=1)&&(maze.can_go(r, c, 'D')))             
                 {V[r+1][c]=1; P[r*C+c]=u; Q.addFirst(new Point(r+1, c));} 
